...
public class DBlockChangeListener implements Listener {
    ...
    public void onPhysics(EntityChangeBlockEvent event) {
        if (event.getEntityType().equals(EntityType.FALLING_BLOCK) && !event.getEntity().getMetadata("splode").isEmpty()) {
            MetadataValue value = Iterables.find(event.getEntity().getMetadata("splode"), new Predicate<MetadataValue>() {
                @Override
                public boolean apply(MetadataValue value) {
                    return value.getOwningPlugin().equals(NorseDemigods.getPlugin(NorseDemigods.class));
                }
            }, null);
            if (value != null) {
